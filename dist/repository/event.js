...
export class EventRepository {
     ...
     */
    async createEvent(params, commitHash) {
     ...
     */
    async updateEvent(params, commitHash, changeType = 'modified') {
     ...
     * Get change history for confidence calculation
     */
    async getChangeHistory(logicalId) {
     ...
     */
    async recalculateConfidence(logicalId) {
    ...
    getChangedFields(current, update) {
        const changed = [];
        if (update.title && update.title !== current.title)
            changed.push('title');
        if (update.description && update.description !== current.description)
            changed.push('description');
        if (update.dateOccurred && update.dateOccurred !== current.dateOccurred)
            changed.push('dateOccurred');
        if (update.statement)
            changed.push('statement');
        if (update.modifiers)
            changed.push('modifiers');
        if (update.relationships)
            changed.push('relationships');
        if (update.source)
            changed.push('source');
        if (update.author && update.author !== current.metadata.author)
            changed.push('author');
        return changed;
    }
    getChangedFieldsBetweenVersions(prev, current) {
        const changed = [];
        if (prev.title !== current.title)
            changed.push('title');
        if (prev.description !== current.description)
            changed.push('description');
        if (prev.dateOccurred !== current.dateOccurred)
            changed.push('dateOccurred');
        if (JSON.stringify(prev.statement) !== JSON.stringify(current.statement))
            changed.push('statement');
        if (JSON.stringify(prev.modifiers) !== JSON.stringify(current.modifiers))
            changed.push('modifiers');
        if (JSON.stringify(prev.relationships) !== JSON.stringify(current.relationships))
            changed.push('relationships');
        if (JSON.stringify(prev.metadata.source) !== JSON.stringify(current.metadata.source))
            changed.push('source');
        if (prev.metadata.author !== current.metadata.author)
            changed.push('author');
        return changed;
    }
}
